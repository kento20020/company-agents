...
会議・開発の各場面で、以下のスキルを必要に応じて使用する。

### 会議フェーズ
| スキル | 用途 | 使うエージェント |
|---|---|---|
| `/deep-research` | Deepリサーチの実行（5+ソース） | 数字ハンター、技術Agent |
| `/exa-search` | Web検索（市場規模、競合、事例） | 数字ハンター、アナロジー職人 |
| `/plan` | 要件の構造化・計画立案 | PM Agent、シンセサイザー |

### 開発フェーズ
| スキル | 用途 | 使うロール |
|---|---|---|
| `/code-architect` | アーキテクチャ設計の実装計画 | Architect |
| `/security-review` | セキュリティ監査 | 監査室 |
| `/code-review` | コード品質チェック | 監査室、Engineer |
| `/tdd-guide` | テスト駆動開発 | Engineer、QA Lead |
| `/build-fix` | ビルドエラー修正 | Engineer |
| `/python-review` | Pythonコードレビュー | Engineer（言語に応じて切替） |
| `/typescript-review` | TypeScriptコードレビュー | Engineer（言語に応じて切替） |

### 使用ルール
- 上記以外のスキルは使わない
- スキルの使用はエージェントの権限に従う（リサーチ権限なしのエージェントは検索スキルを使わない）
- Kenが明示的に別のスキルを要求した場合のみ例外
